...
presetup(){
    ...
    pushd ./artifacts/src/github.com/mediCo_contract_api/go
...
VERSION="1"
CC_SRC_PATH="./artifacts/src/github.com/mediCo/go/"
CC_NAME="mediCo"
...
chaincodeQuery(){
    ...
    # Query all records
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["queryAllRecords"]}'

    # Query by record id
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["queryRecord", "Record-ABCDEEE"]}'
}
